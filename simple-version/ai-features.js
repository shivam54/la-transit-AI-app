// AI Features Module for LA Transit App with Groq API Integration
class AITransitAssistant {
    constructor() {
        // Groq API Configuration - API KEY PLACEHOLDER
        this.groqApiKey = 'YOUR_GROQ_API_KEY_HERE';
        this.groqBaseUrl = 'https://api.groq.com/openai/v1';
        this.model = 'llama-3.3-70b-versatile'; // Groq model (recommended replacement for deprecated llama-3.1-70b-versatile)
        ...
        // Fallback configuration - disabled since API key is hardcoded
        this.useFallback = !this.groqApiKey || localStorage.getItem('force_fallback_mode') === 'true';
    ...
    // API KEY MANAGEMENT - Hardcoded (no longer needed but kept for compatibility)
    getSecureApiKey() {
        // API key is now hardcoded in constructor
    ...
    // Set API key securely (kept for compatibility, but key is now hardcoded)
    setApiKey(apiKey) {
        if (apiKey && (apiKey.startsWith('gsk_') || apiKey.startsWith('sk-'))) {
            // Update the hardcoded key if needed (though it's already set in constructor)
    ...
    // 1. SMART ROUTE PLANNING AI with Groq
    ...
    // 2. ADVANCED CONVERSATIONAL AI with Groq
    ...
    // 3. PREDICTIVE ANALYTICS with Groq
    ...
    // 5. SMART NOTIFICATIONS with Groq
        ...
        try {
            ...
            if (!response.ok) {
                ...
                if (errorMessage.includes('quota') || errorMessage.includes('billing') || errorMessage.includes('exceeded')) {
                    console.warn('Groq API quota exceeded, switching to fallback mode');
            ...
            console.warn('Groq API call failed. Falling back to on-device predictions.', error.message || error);
            throw error;
        }
    }
...
Use REAL-TIME analysis incorporating current weather, traffic, events, and time patterns. Be SPECIFIC about why conditions are what they are right now for THIS PARTICULAR ROUTE.`;
    ...
    parseRouteResponse(response, origin, destination) {
        // Try to extract structured information from Groq AI response
        const routes = [
            {
                mode: 'metro',
    ...
    async getTransitLandRealtime(routeQuery) {
        try {
            ...
            // Safely extract alerts array - handle different response formats
            let alertsArray = [];
            if (Array.isArray(alerts)) {
                alertsArray = alerts;
            } else if (alerts && Array.isArray(alerts.alerts)) {
                alertsArray = alerts.alerts;
            } else if (alerts && alerts.entities && Array.isArray(alerts.entities)) {
                alertsArray = alerts.entities;
            }

            const alertSummaries = alertsArray
                .slice(0, 5)
                .map(a => ({
            ...
            const vehiclesCount = Array.isArray(vehicles) ? vehicles.length : (vehicles.entities || []).length;
            const tripUpdatesCount = Array.isArray(tripUpdates) ? tripUpdates.length : (tripUpdates.entities || []).length;
            const alertsCount = alertsArray.length;
            
            const summary = `Swiftly GTFS-RT → Vehicles: ${vehiclesCount}, TripUpdates: ${tripUpdatesCount}, Alerts: ${alertsCount}`;
    ...
    async getWeatherData(routeCoords = null) {
        try {
            ...
            const response = await fetch(`https://api.openweathermap.org/data/2.5/weather?lat=${location.lat}&lon=${location.lng}&appid=YOUR_OPENWEATHERMAP_API_KEY_HERE&units=imperial`);
    ...
    // API KEY MANAGEMENT UI (deprecated - API key is now hardcoded)
    showApiKeySetup() {
        // This function is no longer used - API key is hardcoded in constructor
        console.log('API key setup UI is deprecated - API key is hardcoded');
    }
}
